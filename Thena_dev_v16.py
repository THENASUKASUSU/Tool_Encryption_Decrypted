...
# --- Fungsi Encrypted/Decryption dengan Algoritma Pilihan (hanya AES-GCM untuk saat ini) ---
    ...
    if args.encrypt or args.decrypt:
        ...
        if args.encrypt:
            ...
            if encryption_success:
                print(f"{GREEN}✅ encrypted selesai: {input_path} -> {created_output}{RESET}")
            else:
                print(f"{RED}❌ encrypted gagal.{RESET}")
                sys.exit(1)
        elif args.decrypt:
            if CRYPTOGRAPHY_AVAILABLE:
                if not os.path.exists(config["master_key_file"]):
                    print(f"{RED}❌ Error: File Master Key '{config['master_key_file']}' tidak ditemukan. Tidak dapat mendecryption tanpanya.{RESET}")
                    sys.exit(1)
                master_key = load_or_create_master_key(password, keyfile_path)
                if master_key is None:
                    print(f"{RED}❌ Gagal mendapatkan Master Key.{RESET}")
            ...
            if decryption_success:
                print(f"{GREEN}✅ Decryption selesai: {input_path} -> {created_output}{RESET}")
            else:
                print(f"{RED}❌ Decryption gagal.{RESET}")
                sys.exit(1)
