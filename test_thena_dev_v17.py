...
class TestThenaDev(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        for file_path in [self.master_key_file, "test.keyfile", "wrong.keyfile", "thena_config.json"]:
            if os.path.exists(file_path):
                os.remove(file_path)

    @patch('builtins.input', return_value='y')
    @patch('Thena_dev_v17.print_box')
    @patch('Thena_dev_v17.print_error_box')
    @patch('Thena_dev_v17.print_loading_progress')
    ...
    def test_main_encrypt_cli(self, mock_load_master_key, mock_encrypt, mock_error, mock_box, mock_input, mock_exit):
        ...
        with patch('sys.argv', ['Thena_dev_v17.py', '--encrypt', '-i', self.input_file, '-o', self.encrypted_file, '-p', 'password']):
            thena.main()
    ...
    def test_main_decrypt_cli(self, mock_load_master_key, mock_decrypt, mock_error, mock_box, mock_input, mock_exit):
        ...
        with patch('sys.argv', ['Thena_dev_v17.py', '--decrypt', '-i', self.encrypted_file, '-o', self.decrypted_file, '-p', 'password']):
            thena.main()
            mock_load_master_key.assert_called_with('password', None)
